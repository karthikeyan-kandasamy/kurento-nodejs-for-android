		...
		switch (message.type) {
			...
			case 'offer':
				if (room.sender && room.sender.websocket) {
					...
					startSendWebRtc(room, message.sdp, onCandidate, function(error, data, from) {
...
function startSendWebRtc(room, sdpOffer, onCandidate, callback) {
	...
	getPipeline(room, function(error, pipeline) {
		...
		createMediaElements(pipeline, ws, function(error, webRtcEndpoint, faceOverlayFilter) {
			...
			sender.endpoint = webRtcEndpoint;
			...
			connectMediaElements(webRtcEndpoint, faceOverlayFilter, function(error) {
				...
				faceOverlayFilter.on('FaceDetected', function (data){
				    return callback(error, data, 2);
				});
				...
				sender.endpoint.processOffer(sdpOffer, function(error, sdpAnswer) {
					...
					return callback(null, sdpAnswer, 1);
				});
			});
...
function createMediaElements(pipeline, ws, callback) {
    pipeline.create('WebRtcEndpoint', function(error, webRtcEndpoint) {
        ...
        pipeline.create('OpencvPluginSample', function(error, faceOverlayFilter) {
            ...
            faceOverlayFilter.setFilterType(1, function(error) {
                if (error) {
                    return callback(error);
                }
